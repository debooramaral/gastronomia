...
export default class OrdersDataAccess {
    ...
    try {
      const result = await Mongo.db
        ...
        .aggregate([
          ...
          {
            $project: {
              //Não passar os dados sensiveis de usuário (password e salt)
          ...
          {
            $group: {
              //englobar, juntar todos os itens de um mesmo pedido
              _id: "$_Id",
  ...
  async getOrdersByUserId(userId) {
    //pegar as ordens de um determinado usuario
    try {
      const result = await Mongo.db
        .collection(collectionName)
        .aggregate([
          //especificar que é pelo ID de usuário
          {
